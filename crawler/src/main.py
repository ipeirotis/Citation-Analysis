...
def fetch_author_data():
    ...
    task = taskqueue.Task(url='/crawlAuthor', params={'authorId': author, 'uni': uni, 'dpt': dpt})
    q_auth.add(task)


    return "fetching author", 202
...
def crawl_author():
    ...
    uni = form['uni']
    dpt = form['dpt']
    author_obj = authorCrawler.authorCrawler(author, uni, dpt, 3)
    return "author fetched", 200

@app.route('/author/<author_id>', defaults={'uni': None, 'dpt': None},  methods=['GET'])
@app.route('/author/<author_id>/<uni>/<dpt>', methods=['GET'])
def retrieve_author_data(author_id, uni, dpt):
    if author_id is None:
        ...
        if author_data:
            ...
            if nocache == "yes":
                ...
                data = urllib.urlencode({ 'authorId': author_id, 'uni': uni, 'dpt':dpt })
                ...
                try:
                    ...
                    return 'waiting for author to be fetched, retry later', 202
            ...
            if not nocache or nocache == "no":
                ...
                if last_time_retrieved:
                    ...
                    for row in iter(author_data):
                        ...
                        out['university'] = row[6]
                        out['department'] = row[7]
                        out['coauthor_ids'].append(row[8])
    ...
    if not pub_id:
        ...
        if pub_data:
            ...
            try:
                ...
                return 'waiting for publication to be fetched, retry later', 202
...
def fetch_publication_data():
    ...
    return "fetching publication", 202
...
def crawl_publication():
    ...
    db.close()
    return "publication fetched", 200
...
def init_db():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS scholar_db.authors ( \
                   ...
                   university VARCHAR(45) COLLATE utf8_unicode_ci NULL, \
                   department VARCHAR(45) COLLATE utf8_unicode_ci NULL, \
